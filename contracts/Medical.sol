...
contract Healthcare {
    ...
    function ADD_MEDICINE(
        ...
        ADD_NOTIFICATION(
            msg.sender,
            "New Medicine added to marketplace successfully!",
            "Medicine"
        );
    ...
    function UPDATE_MEDICINE_LOCATION(
        uint _medicineId,
        ...
        ADD_NOTIFICATION(
            msg.sender,
            "Medicine location updated successfully",
            "Medicine"
        );
    ...
    function UPDATE_MEDICINE_PRICE(
        uint _medicineId,
        uint _price
    ) public onlyAdmin {
        require(_medicineId <= medicineCount, "Medicine does not exist");
    ...
    function UPDATE_MEDICINE_QUANTITY(
        uint _medicineId,
        uint _quantity
    ) public onlyAdmin {
        require(_medicineId <= medicineCount, "Medicine does not exist");
    ...
    function UPDATE_MEDICINE_DISCOUNT(
        uint _medicineId,
        uint _discount
    ) public onlyAdmin {
        require(_medicineId <= medicineCount, "Medicine does not exist");
        ...
        ADD_NOTIFICATION(
            msg.sender,
            "Medicine discount updated successfully",
            "Medicine"
        );
    ...
    function ADD_DOCTOR(
        ...
        require(
            msg.value == registrationDoctorFee,
        ...
        ADD_NOTIFICATION(
            _address,
            "You have successfully completed the registration, now wating for approval",
            "Doctor"
        );
        ADD_NOTIFICATION(
            admin,
            "New doctor is registor, now wating for approval",
            "Doctor"
        );
    ...
    function APPROVE_DOCTOR_STATUS(uint _doctorId) public onlyAdmin {
        ...
        ADD_NOTIFICATION(
            msg.sender,
            "You have approved Docotr registration",
            "Doctor"
        );
        ADD_NOTIFICATION(
            doctors[_doctorId].accountAddress,
    ...
    function DISAPPROVE_DOCTOR_STATUS(uint _doctorId) public onlyAdmin {
        require(_doctorId <= doctorCount, "Doctor does not exist");
        require(doctors[_doctorId].isApproved, "Doctor is already disapproved");

        doctors[_doctorId].isApproved = false;

        ADD_NOTIFICATION(
            msg.sender,
            "You have disapproved Doctor registration",
            "Doctor"
        );
        ADD_NOTIFICATION(
            doctors[_doctorId].accountAddress,
            "Your registration is disapproved",
            "Doctor"
        );
    ...
    // UPDATE BY DOCTOR
    function UPDATE_PATIENT_MEDICAL_HISTORY(
        uint _patientId,
        ...
        ADD_NOTIFICATION(
            msg.sender,
            "You have successfully update, patient medical history",
            "Doctor"
        );

        ADD_NOTIFICATION(
            patients[_patientId].accountAddress,
            "Your medical history updated by doctor",
            "Doctor"
        );

        ADD_NOTIFICATION(
            msg.sender,
            "Patient medicial history is updated",
            "Doctor"
        );
    }
    ...
    function COMPLETE_APPOINTMENT(uint _appointmentId) public onlyDoctor {
        require(
            _appointmentId <= appointmentCount,
        ...
        ADD_NOTIFICATION(
            msg.sender,
            "You have successfully completed the appointment",
            "Doctor"
        );

        ADD_NOTIFICATION(
            patients[appointments[_appointmentId].patientId].accountAddress,
        ...
        ADD_NOTIFICATION(
            admin,
            "Doctor completed appointment successfully",
            "Doctor"
        );
    }

    function PRESCRIBE_MEDICINE(
        uint _medicineId,
        uint _patientId
    ) public onlyDoctor {
        require(
            doctors[GET_DOCTOR_ID(msg.sender)].isApproved,
        ...
        ADD_NOTIFICATION(
            msg.sender,
            "You have successfully prescribed medicine",
            "Doctor"
        );

        ADD_NOTIFICATION(
            patients[_patientId].accountAddress,
            "Doctor prescribed you medicine",
            "Doctor"
        );

        ADD_NOTIFICATION(
            admin,
            "Doctor prescribed medicine successfully",
            "Doctor"
        );
    }
    ...
    function ADD_PATIENTS(
        ...
        require(
            msg.value == registrationPatientFee,
            "Incorrect registration fee"
        );
        require(
            !registeredPatients[_accountAddress],
        ...
        ADD_NOTIFICATION(
            admin,
            "New Patient is registor successfully",
            "Patient"
        );
    ...
    function BOOK_APPOINTMENT(
        ...
        require(
            ...
            "Only the patient can book their appointment"
        );
    ...
    function BUY_MEDICINE(
        ...
        require(
            patients[_patientId].accountAddress == msg.sender,
            "Only the patient can buy their medicine"
        );
        ...
        ADD_NOTIFICATION(
            msg.sender,
            "You have successfully bought medicine",
            "Patient"
        );

        ADD_NOTIFICATION(
            admin,
            "Transaction completed, madicine bought successfully",
            "Patient"
        );
    ...
    function UPDATE_REGISTRATION_FEE(uint _newFee) public onlyAdmin {
        ...
        ADD_NOTIFICATION(
            admin,
            "You have successfully updated Registration fee",
            "Admin"
        );
    }
    ...
    function UPDATE_APPOINTMENT_FEE(uint _newFee) public onlyAdmin {
        ...
        ADD_NOTIFICATION(
            admin,
            "You have successfully updated Appointment fee",
            "Admin"
        );
    }
    ...
    function UPDATE_REGISTRATION_PATIENT_FEE(uint _newFee) public onlyAdmin {
        ...
        ADD_NOTIFICATION(
            admin,
            "You have successfully updated Patient Registration fee",
            "Admin"
        );
    }
    ...
    function UPDATE_ADMIN_ADDRESS(
        ...
        ADD_NOTIFICATION(
            admin,
            "You have successfully updated admin address",
            "Admin"
        );
    }
    ...
    function GET_ALL_PATIENT_ORDERS(
        uint patientId
    ...
    function GET_ALL_PRESCRIBED_MEDICINES()
        public
        view
        returns (Prescription[] memory)
    {
        Prescription[] memory allPrescriptions = new Prescription[](
            prescriptionCount
        );
        uint counter = 0;
    ...
    function GET_ALL_PRESCRIBED_MEDICINES_OF_PATIENT(
        uint patientId
    ) public view returns (Prescription[] memory) {
        uint count = 0;
        for (uint i = 1; i <= prescriptionCount; i++) {
    ...
    function GET_ALL_REGISTERED_PATIENTS()
        public
        view
    ...
    function GET_PATIENT_APPOINTMENT(
        uint _appointmentId
        ...
        require(
            appointments[_appointmentId].patientId ==
                GET_PATIENT_ID(msg.sender) ||
                msg.sender == admin,
            "Only the patient or admin can view the appointment"
        );
        return appointments[_appointmentId];
    }

    function GET_PATIENT_MEDICIAL_HISTORY(
        uint _patientId
        ...
        require(
            patients[_patientId].accountAddress == msg.sender ||
                msg.sender == admin,
            "Only the patient or admin can view the medical history"
        );
        return patients[_patientId].medicalHistory;
    }

    function GET_PATIENT_APPOINTMENT_HISTORYS(
        uint _patientId
    ) public view returns (Appointment[] memory) {
        require(_patientId <= patientCount, "Patient does not exist");
    ...
    function GET_BOUGHT_MEDICINE_BY_PAITENT(
        uint _patientId
    ...
    function GET_MOST_POPULAR_DOCTOR() public view returns (Doctor memory) {
        ...
        uint highestScore = 0;
        for (uint i = 1; i <= doctorCount; i++) {
            uint score = doctors[i].appointmentCount +
                doctors[i].successfulTreatmentCount;
    ...
    function GET_DOCTOR_APPOINTMENTS_HISTORYS(
        uint _doctorId
    ) public view returns (Appointment[] memory) {
        require(_doctorId <= doctorCount, "Doctor does not exist");
    ...
    function GET_ALL_REGISTERED_MEDICINES()
        public
        view
    ...
    function GET_MEDICINE_DETAILS(
        uint _medicineId
    ) public view returns (Medicine memory) {
    ...
    function _SEND_MESSAGE(
        ...
        require(CHECK_USER_EXISTS(friend_key), "User is not registered");
        require(
            CHECK_ALREADY_FRIENDS(_myAddress, friend_key),

...
export const StateContextProvider = ({ children }) => {
  ...
  const DISAPPROVE_DOCTOR_STATUS = async (_doctorId) => {
    try {
      if (!_doctorId) return notifyError("Data missing");

      setLoader(true);
      notifySuccess("Registration Disapproval is processing... ");

      const address = await CHECKI_IF_CONNECTED_LOAD();
      if (address) {
        const contract = await HEALTH_CARE_CONTARCT();

        const transaction = await contract.DISAPPROVE_DOCTOR_STATUS(
          Number(_doctorId),
          {
            gasLimit: ethers.utils.hexlify(8000000),
          }
        ...
        if (transaction.hash) {
          setLoader(false);
          notifySuccess("Registration Disapproval complete");
          setReCall(reCall + 1);
        }
      }
    } catch (error) {
